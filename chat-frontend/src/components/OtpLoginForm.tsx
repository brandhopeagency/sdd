...
export default function OtpLoginForm({
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setSendCooldown(prev => Math.max(0, prev - 1)), 1000);
    return () => clearInterval(id);
  }, [sendCooldown]);

  useEffect(() => {
    if (verifyCooldown <= 0) return;
    const id = setInterval(() => setVerifyCooldown(prev => Math.max(0, prev - 1)), 1000);
    return () => clearInterval(id);
  }, [verifyCooldown]);

  useEffect(() => {
    if (otpError === 'rate_limited') {
      if (otpSent) {
    ...
    return (
      <form onSubmit={handleSendOtp} className="space-y-4">
        ...
        <div className={compact ? 'flex gap-2' : 'space-y-2'}>
          ...
          >
            {sendCooldown > 0 ? (
              t('login.otp.rate_limited_send', { seconds: sendCooldown, defaultValue: `Please wait ${sendCooldown}s` })
            ) : isLoading ? (
              <>
  ...
  return (
    <form onSubmit={handleVerifyOtp} className="space-y-4">
      ...
      <div className={compact ? 'flex gap-2' : 'space-y-2'}>
        <button
          type="submit"
          disabled={isLoading || otp.length !== 6 || verifyCooldown > 0}
          className={`btn-primary ${compact ? 'flex-1' : 'w-full'} py-3`}
        >
          {verifyCooldown > 0 ? (
            t('login.otp.rate_limited', { seconds: verifyCooldown, defaultValue: `Too many attempts. Wait ${verifyCooldown}s` })
          ) : isLoading ? (
            <>
              <Loader2 className="w-4 h-4 mr-2 animate-spin" />
