...
/**
 * Wrapper over centralized API fetch/retry logic.
 */
async function fetchWithRetry(
  ...
  return apiFetch(url, options, retryOnAuth);
}
...
export const useChatStore = create<ChatState>((set, get) => ({
    ...
    try {
      ...
      const response = await fetchWithRetry(`/api/chat/sessions`, {
        ...
        body: JSON.stringify({
          ...
          // Guest sessions must pass guest userId. For authenticated users with a missing/expired
          // token, pass userId as well so backend returns 401 (not guest-disabled 403), enabling
          // refresh-and-retry flow in fetchWithRetry().
          ...(typeof effectiveUserId === 'string' && (
            effectiveUserId.startsWith('guest_') || !token
          )
  ...
  fetchCurrentUserMemory: async (opts?: { force?: boolean }) => {
    ...
    const response = await fetchWithRetry(`/api/chat/memory`, {
      method: 'GET',
      headers,
  ...
  refreshSessionMemory: async (sessionId: string) => {
    ...
    const response = await fetchWithRetry(`/api/chat/sessions/${sessionId}/memory/refresh`, {
      method: 'POST',
      headers,
      body: JSON.stringify(
  ...
  endSessionInBackground: async (sessionId: string) => {
    ...
    try {
      ...
      const response = await fetchWithRetry(`/api/chat/sessions/${sessionId}/end`, {
        method: 'POST',
        headers: {
          ...headers,
          'Content-Type': 'application/json'
  ...
  endSession: async () => {
    ...
    try {
      ...
      const response = await fetchWithRetry(`/api/chat/sessions/${session.id}/end`, {
        method: 'POST',
        headers: {
          ...headers,
          'Content-Type': 'application/json'
  ...
  submitFeedback: async (messageId: string, rating: 1 | 2 | 3 | 4 | 5, comment?: string) => {
    ...
    try {
      ...
      const response = await fetchWithRetry(`/api/chat/messages/${messageId}/feedback`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          rating,
