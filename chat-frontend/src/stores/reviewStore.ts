...
interface ReviewState {
  ...
  queueTab: 'pending' | 'flagged' | 'in_progress' | 'completed' | 'excluded';
  queuePage: number;
  queueFilters: {
    riskLevel?: string;
    language?: string;
  ...
  setQueueTab: (tab: 'pending' | 'flagged' | 'in_progress' | 'completed' | 'excluded') => void;
...
export const useReviewStore = create<ReviewState>()((set, get) => ({
  ...
  setQueueTab: (tab) => {
    if (tab === 'excluded') {
      ...
      set({ queueTab: tab as 'pending' | 'flagged' | 'in_progress' | 'completed', showExcluded: false, queuePage: 1 });
    }
    get().fetchQueue();
  },
