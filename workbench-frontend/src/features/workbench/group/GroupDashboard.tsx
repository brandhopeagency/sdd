  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const resp = await groupAdminApi.dashboard(resolvedGroupId);
        if (!mounted) return;
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            {canAccessGroupModeration && (
              <div
                className="card p-5 cursor-pointer hover:shadow-md transition-shadow"
                onClick={() => navigate('/workbench/group/sessions')}
              >
                <div className="flex items-center justify-between">
                  <div>
            ...
            {canAccessGroupModeration && (
              <div
                className="card p-5 cursor-pointer hover:shadow-md transition-shadow"
                onClick={() => navigate('/workbench/group/sessions?status=pending')}
              >
                <div className="flex items-center justify-between">
                  <div>
