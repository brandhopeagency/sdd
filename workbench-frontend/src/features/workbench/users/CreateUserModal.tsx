...
import { usersApi, groupAdminApi } from '@/services/adminApi';
import { UserRole } from '@mentalhelpglobal/chat-types';
...
export default function CreateUserModal({ onSuccess, onClose, groupId }: CreateUserModalProps) {
    ...
    try {
      ...
      if (response.success) {
        if (groupId && response.data?.id) {
          try {
            await groupAdminApi.addUser(groupId, { userId: response.data.id });
          } catch {
            // User created but group-add failed; still consider success
          }
        }
