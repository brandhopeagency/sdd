...
import { adminAuditApi, adminSettingsApi } from '@/services/adminApi';
...
export default function SettingsView() {
  ...
  const [adminSettings, setAdminSettings] = useState<{ guestModeEnabled: boolean; approvalCooloffDays: number; otpLoginDisabledWorkbench: boolean } | null>(
    null
  );
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {isOwner && (
        <>
        <div className="my-8 border-t-2 border-neutral-200" />
        <div className="card p-6 border-amber-200/60">
          ...
          {adminSettings && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-4 bg-neutral-50 rounded-lg">
                <div>
                  <h3 className="font-medium text-neutral-900">{t('settings.admin.otpDisable')}</h3>
                  <p className="text-sm text-neutral-500">{t('settings.admin.otpDisableDesc')}</p>
                </div>
                <button
                  onClick={() =>
                    setAdminSettings((prev) =>
                      prev ? { ...prev, otpLoginDisabledWorkbench: !prev.otpLoginDisabledWorkbench } : prev
                    )
                  }
                  className={`relative w-14 h-8 rounded-full transition-colors overflow-hidden shrink-0 ${
                    adminSettings.otpLoginDisabledWorkbench ? 'bg-secondary-500' : 'bg-neutral-300'
                  }`}
                  aria-label={t('settings.admin.otpDisable')}
                >
                  <span
                    className={`absolute top-1 left-1 w-6 h-6 bg-white rounded-full shadow transition-transform ${
                      adminSettings.otpLoginDisabledWorkbench ? 'translate-x-6' : 'translate-x-0'
                    }`}
                  />
                </button>
              </div>
              <div className="flex justify-end">
                <button
                  className="btn-outline border-amber-300 text-amber-700 hover:bg-amber-50 hover:border-amber-400"
