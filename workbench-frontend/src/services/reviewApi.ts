...
import type {
  ...
  SupervisorReview, SupervisorDecisionInput,
  SupervisionQueueItem, AwaitingFeedbackItem,
  GradeDescription, UpdateGradeDescriptionInput,
  GroupReviewConfig, UpdateGroupReviewConfigInput,
...
export async function getSupervisionQueue(): Promise<SupervisionQueueItem[]> {
  return apiGet<SupervisionQueueItem[]>('/supervision/queue');
}
...
export async function getSupervisionContext(sessionReviewId: string): Promise<{
  ...
  return apiGet(`/supervision/${sessionReviewId}`);
}

export async function submitSupervisionDecision(
  sessionReviewId: string,
  input: SupervisorDecisionInput,
): Promise<SupervisorReview> {
  return apiPost<SupervisorReview>(`/supervision/${sessionReviewId}/decision`, input);
}

export async function getAwaitingFeedback(reviewerId: string): Promise<AwaitingFeedbackItem[]> {
  return apiGet<AwaitingFeedbackItem[]>(`/supervision/awaiting/${reviewerId}`);
}
...
export async function getGradeDescriptions(): Promise<GradeDescription[]> {
  return apiGet<GradeDescription[]>('/grade-descriptions');
}
...
export async function updateGradeDescription(
  ...
  return apiPut<GradeDescription>(`/grade-descriptions/${scoreLevel}`, input);
}
...
export async function getGroupReviewConfig(groupId: string): Promise<GroupReviewConfig | null> {
  try {
    return await apiGet<GroupReviewConfig>(`/admin/review/groups/${groupId}/config`);
  } catch {
    return null;
  }
}

export async function updateGroupReviewConfig(
  groupId: string,
  input: UpdateGroupReviewConfigInput,
): Promise<GroupReviewConfig> {
  const res = await fetchWithAuth(`${ADMIN_REVIEW_BASE}/groups/${groupId}/config`, {
    method: 'PUT',
    body: JSON.stringify(input),
  });
  const json = await res.json();
...
export async function deleteGroupReviewConfig(groupId: string): Promise<void> {
  await fetchWithAuth(`${ADMIN_REVIEW_BASE}/groups/${groupId}/config`, { method: 'DELETE' });
}
