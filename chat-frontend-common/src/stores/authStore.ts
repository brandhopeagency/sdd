...
import { authApi, settingsApi, setAccessToken, clearTokens, getAccessToken, setAuthRefreshHandlers } from '../services/api';

function resolvePreferredGroupId(user: AuthenticatedUser): string | null {
  const memberships = user.memberships || [];
  return user.activeGroupId ?? memberships.find((m) => m.status === 'active')?.groupId ?? memberships[0]?.groupId ?? null;
}
...
interface AuthState {
  ...
  // OTP actions
  sendOtp: (email: string, surface?: 'chat' | 'workbench') => Promise<boolean>;
  verifyOtp: (
    email: string,
    code: string,
  ...
  googleLogin: (credential: string, surface: 'chat' | 'workbench', invitationCode?: string) => Promise<{ success: boolean; isNewUser: boolean; user?: AuthenticatedUser; pendingApproval?: boolean }>;
  loadGoogleConfig: () => Promise<void>;
...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      sendOtp: async (email: string, surface?: 'chat' | 'workbench') => {
        set({ isLoading: true, otpError: null });
        
        try {
          const response = await authApi.sendOtp(email, surface);
      ...
      verifyOtp: async (email: string, code: string, invitationCode?: string) => {
        ...
        try {
          ...
          if (response.success && response.data) {
            ...
            const defaultGroupId = resolvePreferredGroupId(response.data.user);
      ...
      googleLogin: async (credential: string, surface: 'chat' | 'workbench', invitationCode?: string) => {
        set({ isLoading: true, otpError: null });
        try {
          const response = await authApi.googleLogin(credential, surface, invitationCode);

          if (response.success && response.data) {
            if (response.data.status === 'approval' || response.data.status === 'pending') {
              set({ isLoading: false });
              return { success: false, isNewUser: true, pendingApproval: true };
            }
          ...
          const errorCode = response.error?.code?.toLowerCase() || 'google_login_failed';
          set({ isLoading: false, otpError: errorCode });
          return { success: false, isNewUser: false };
        } catch (error) {
          console.error('[Auth] Google login error:', error);
          set({ isLoading: false, otpError: 'google_login_failed' });
          return { success: false, isNewUser: false };
        }
      },

      loadGoogleConfig: async () => {
        try {
          const response = await authApi.getGoogleConfig();
          if (response.success && response.data) {
            set({
              googleClientId: response.data.clientId,
              googleOAuthAvailable: response.data.available
            });
          }
        } catch (error) {
          console.warn('[Auth] Failed to load Google config:', error);
        }
      },
      ...
      upgradeFromGuest: (authUser: AuthenticatedUser) => {
        ...
        const defaultGroupId = resolvePreferredGroupId(authUser);
        set({
          user: authUser,
          isAuthenticated: true,
          isGuest: false,
      ...
      // Handles cross-surface SSO: when no local access token exists, a
      // silent refresh is attempted using the parent-domain cookie so a
      // user authenticated on one surface is recognised on the other.
      initializeAuth: async () => {
        const token = getAccessToken();

        if (!token) {
          // No local token – attempt a silent cookie-based refresh.
          // The refresh endpoint sends `credentials: 'include'`, which
          // attaches the parent-domain refresh-token cookie automatically.
          set({ isLoading: true });
          try {
        ...
        try {
          ...
          if (response.success && response.data) {
            const defaultGroupId = resolvePreferredGroupId(response.data);
            set({
              user: response.data,
              isAuthenticated: true,
              isLoading: false,
        ...
        try {
          ...
          if (response.success && response.data) {
            set({
              ...
              otpLoginDisabledWorkbench: response.data.otpLoginDisabledWorkbench ?? false,
              googleOAuthAvailable: response.data.googleOAuthAvailable ?? false
            });
          }
        } catch (error) {
...
// Wire the auth store's refresh logic into the API fetch layer
// so that 401/403 responses trigger automatic token refresh.
setAuthRefreshHandlers({
  refreshSession: () => useAuthStore.getState().refreshSession(),
  handleApiError: (err) => useAuthStore.getState().handleApiError(err),
});
