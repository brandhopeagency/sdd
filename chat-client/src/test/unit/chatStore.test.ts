...
describe('chatStore', () => {
  it('startSession: uses backend session + sets greeting (guest passes userId)', async () => {
    ...
    vi.doMock('@/config', () => ({ API_BASE_URL: 'http://api' }))
    vi.doMock('@/i18n', () => ({ default: { language: 'en' } }))

    const fetchMock = vi.fn(async (url: any, init?: any) => {
      expect(String(url)).toContain('http://api/chat/sessions')
  ...
  it('sendMessage: sends message and replaces temp message with server messages', async () => {
    ...
    vi.doMock('@/services/api', () => ({ getAccessToken: () => null }))
    vi.doMock('@/config', () => ({ API_BASE_URL: 'http://api' }))
    vi.doMock('@/i18n', () => ({ default: { language: 'en' } }))

    const fetchMock = vi.fn(async (url: any, init?: any) => {
      expect(String(url)).toContain('http://api/chat/message')
      expect(init?.method).toBe('POST')
      return jsonResponse({
        data: {
          userMessage: {
