...
import { saveConversation } from './gcs.service';
 ...
 * - inactivity timeout expiry based on sessions.updated_at
 *
...
async function ensureSessionPersisted(sessionId: string): Promise<boolean> {
  ...
  try {
    await pool.query(
      `INSERT INTO sessions (id, user_id, guest_id, group_id, dialogflow_session_id, status, started_at, message_count, language_code, created_at, updated_at)
