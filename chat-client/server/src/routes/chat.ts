    ...
    if (req.user?.id) {
      ...
      if (requestUserId) {
        ...
        if (typeof requestUserId === 'string' && requestUserId.startsWith('guest_')) {
          ...
          // If the client is sending a real (UUID) userId without a valid token, treat it as auth-required.
          // This enables token refresh/retry behavior on the client (401 triggers refresh), and avoids hard 400s
          // from older clients that sent userId in the body.
...
router.post('/sessions/:id/end', optionalAuthActiveAccount, async (req: Request, res: Response) => {
  try {
    ...
    if (!isGuestSession && !isOwnSession) {
      ...
      return res.status(403).json({
        success: false,
        error: 'Access denied: You can only end your own sessions'
      });
    }
...
router.post('/messages/:id/feedback', optionalAuthActiveAccount, async (req: Request, res: Response) => {
  try {
    ...
    if (!isGuestSession && !isOwnSession) {
      return res.status(403).json({
        success: false,
        error: 'Access denied: You can only submit feedback for your own sessions'
      });
    }
