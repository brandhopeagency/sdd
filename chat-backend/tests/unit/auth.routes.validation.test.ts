...
const authenticateWithGoogleMock = vi.fn()
...
const requestMembershipWithInviteCodeMock = vi.fn()
...
vi.mock('../../src/services/auth.service', () => ({
  ...
  authenticateWithGoogle: (...args: any[]) => authenticateWithGoogleMock(...args),
...
vi.mock('../../src/services/groupMembership.service', () => ({
  requestMembershipWithInviteCode: (...args: any[]) => requestMembershipWithInviteCodeMock(...args),
}))

vi.mock('../../src/services/settings.service', () => ({
  getSettings: vi.fn(async () => ({ approvalCooloffDays: 7, otpLoginDisabledWorkbench: false })),
}))

vi.mock('../../src/services/user.service', () => ({
  markUserForApproval: vi.fn(),
}))

vi.mock('../../src/services/google-auth.service', () => ({
...
describe('auth routes: validation + error mapping', () => {
  beforeEach(() => {
    ...
    authenticateWithGoogleMock.mockReset()
    ...
    requestMembershipWithInviteCodeMock.mockReset()
  ...
  it('POST /api/auth/otp/verify: issues tokens after invite auto-admit activates pending user', async () => {
    verifyOtpMock.mockResolvedValueOnce({ valid: true })
    authenticateWithOtpMock.mockResolvedValueOnce({
      user: { id: 'u1', email: 'invite@user.com' },
      isNewUser: true,
      status: 'pending',
      accessToken: undefined,
      refreshToken: undefined,
    })
    requestMembershipWithInviteCodeMock.mockResolvedValueOnce({ groupId: 'g1', status: 'active' })
    getUserByIdMock
      .mockResolvedValueOnce({ id: 'u1', status: 'pending', email: 'invite@user.com' })
      .mockResolvedValueOnce({ id: 'u1', status: 'active', email: 'invite@user.com' })
    generateAccessTokenMock.mockReturnValueOnce('minted-access')
    generateRefreshTokenMock.mockResolvedValueOnce('minted-refresh')

    const { server, baseUrl } = await startAuthApp()
    try {
      const resp = await fetch(`${baseUrl}/api/auth/otp/verify`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ email: 'invite@user.com', code: '123456', invitationCode: 'GATETEST01' }),
      })
      expect(resp.status).toBe(200)
      const body = await resp.json()
      expect(body.success).toBe(true)
      expect(body.data.accessToken).toBe('minted-access')
      const setCookie = resp.headers.get('set-cookie') || ''
      expect(setCookie.toLowerCase()).toContain('refreshtoken=')
      expect(generateAccessTokenMock).toHaveBeenCalledTimes(1)
      expect(generateRefreshTokenMock).toHaveBeenCalledWith('u1')
    } finally {
      server.close()
    }
  })
