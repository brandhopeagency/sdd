...
export class ConsoleEmailProvider implements EmailProvider {
  ...
  async sendEmail(options: { to: string; subject: string; text: string }): Promise<boolean> {
    const timestamp = new Date().toISOString();
    console.log('');
    console.log('╔══════════════════════════════════════════════════════════════╗');
    console.log('║                    📧 ALERT EMAIL (Console)                 ║');
    console.log('╠══════════════════════════════════════════════════════════════╣');
    console.log(`║  To:      ${options.to.padEnd(50)}║`);
    console.log(`║  Subject: ${options.subject.substring(0, 50).padEnd(50)}║`);
    console.log(`║  Time:    ${timestamp.padEnd(50)}║`);
    console.log('╚══════════════════════════════════════════════════════════════╝');
