...
export async function listGroupSessions(groupId: string, params: GroupSessionListParams) {
  ...
  const where: string[] = [
    `(s.group_id = $1 OR (s.group_id IS NULL AND s.user_id IN (SELECT gm.user_id FROM group_memberships gm WHERE gm.group_id = $1 AND gm.status = 'active')))`
  ];
...
export async function getGroupSessionById(groupId: string, sessionId: string): Promise<GroupSessionRow | null> {
  ...
  const result = await pool.query(
    `SELECT * FROM sessions WHERE id = $1 AND (group_id = $2 OR (group_id IS NULL AND user_id IN (SELECT gm.user_id FROM group_memberships gm WHERE gm.group_id = $2 AND gm.status = 'active')))`,
    [sessionId, groupId]
  );
