...
export async function createAndAddUserToGroup(
  input: {
    groupId: string;
    email: string;
    displayName: string;
  ...
  const userResult = await query<DbUser>(
    `INSERT INTO users (email, display_name, role, status)
     VALUES ($1, $2, $3, 'active')
     RETURNING *`,
    [normalizedEmail, input.displayName.trim(), input.role || 'user']
  );
  const newUser = dbUserToUser(userResult.rows[0]);
...
export async function checkEmailExists(email: string): Promise<{ exists: boolean; userId?: string; displayName?: string }> {
  ...
  return { exists: true, userId: result.rows[0].id, displayName: result.rows[0].display_name };
}
...
export async function createInvitationCode(
  input: { groupId: string; code?: string; expiresAt?: string | null; maxUses?: number; requiresApproval?: boolean },
  actorId: string,
  ipAddress?: string
  ...
  const result = await query(
    `INSERT INTO group_invite_codes (group_id, code, created_by, expires_at, max_uses, uses, requires_approval, metadata)
