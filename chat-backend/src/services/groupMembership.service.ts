...
export async function createInviteCode(
  input: { groupId: string; maxUses?: number; expiresAt?: string | null; requiresApproval?: boolean },
  actorId: string,
  ipAddress?: string
): Promise<{ code: string; groupId: string; maxUses: number; expiresAt: Date | null; requiresApproval: boolean }> {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try {
      await query(
        `
          INSERT INTO group_invite_codes (group_id, code, created_by, max_uses, expires_at, requires_approval)
          VALUES ($1, $2, $3, $4, $5, $6)
        `,
        [input.groupId, code, actorId, maxUses, expiresAt, requiresApproval]
      );

      await logAuditEvent(actorId, 'group.invite_create', 'group', input.groupId, { code, maxUses, expiresAt, requiresApproval }, ipAddress);
      return { code, groupId: input.groupId, maxUses, expiresAt, requiresApproval };
    } catch (e: any) {
...
export async function requestMembershipWithInviteCode(
  ...
  return transaction(async (client) => {
    ...
    const invite = await client.query<{
      ...
      `
      SELECT id, group_id, revoked_at, expires_at, max_uses, uses, COALESCE(requires_approval, true) AS requires_approval
    ...
    const autoAdmit = row.requires_approval === false;
    const membershipStatus: GroupMembershipStatus = autoAdmit ? 'active' : 'pending';

    await client.query(
      `
      INSERT INTO group_memberships (user_id, group_id, role, status, requested_by, approved_by, approved_at, metadata)
      VALUES ($1, $2, 'member', $3, $1, $4, $5, $6::jsonb)
      `,
      [
        input.userId,
        row.group_id,
        membershipStatus,
        autoAdmit ? null : null,
        autoAdmit ? new Date() : null,
        JSON.stringify({
          source: 'invite',
          inviteCodeId: row.id,
          ipAddress: ipAddress ?? null,
          requestedAt: new Date().toISOString(),
          ...(autoAdmit ? { autoAdmitted: true } : {})
        })
      ]
    );
    ...
    if (autoAdmit) {
      await client.query(
        `UPDATE group_invite_codes SET uses = uses + 1 WHERE id = $1`,
        [row.id]
      );
      await client.query(
        `UPDATE users SET status = 'active', active_group_id = COALESCE(active_group_id, $1) WHERE id = $2 AND status = 'pending'`,
    ...
    return { groupId: row.group_id, status: membershipStatus };
  });
}
...
export async function approveGroupRequest(
  ...
  return transaction(async (client) => {
    ...
    await client.query(`UPDATE users SET active_group_id = COALESCE(active_group_id, $1) WHERE id = $2`, [
      input.groupId,
      input.userId
    ]);
