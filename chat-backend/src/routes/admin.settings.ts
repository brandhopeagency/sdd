...
router.patch('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const otpLoginDisabledWorkbench =
      typeof req.body?.otpLoginDisabledWorkbench === 'boolean' ? req.body.otpLoginDisabledWorkbench : undefined;
    ...
    const previous = await getSettings(true);
    const updated = await updateSettings({
      ...(guestModeEnabled !== undefined ? { guestModeEnabled } : {}),
      ...(approvalCooloffDays !== undefined ? { approvalCooloffDays } : {}),
      ...(otpLoginDisabledWorkbench !== undefined ? { otpLoginDisabledWorkbench } : {})
    });

    if (otpLoginDisabledWorkbench !== undefined && otpLoginDisabledWorkbench !== previous.otpLoginDisabledWorkbench) {
      await logAuditEvent(
        req.userId || null,
        'settings_otp_disable_workbench_changed',
        'settings',
        '1',
        { previous: previous.otpLoginDisabledWorkbench, current: otpLoginDisabledWorkbench },
        getClientIp(req)
      );
    }
